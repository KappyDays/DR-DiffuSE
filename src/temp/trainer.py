...
#             self.model = model
#         else:
#             self.train_loader = ddp_data_loader(train_data, opt, 'train', collate.collate_fn)
#             self.valid_loader = ddp_data_loader(valid_data, opt, 'valid', collate.collate_fn)
...
# class BaseTrainer(BasicTrainer):
#     def __init__(self, train_data, valid_data, model, logger, opt):
#         super(BaseTrainer, self).__init__(train_data, valid_data, model, logger, opt)
            ...
            for epoch in range(self.opt.n_epoch):
                ...
                for batch in self.train_loader:
                    ...
                    for key in batch.keys():
                        try:
                            batch[key] = batch[key].to(self.opt.local_gpu_id)
                    ...
                    if self.opt.wandb and is_main_process():
                        # if self.opt.rank == 0:
                        wandb.log(
                            {
